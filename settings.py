...
window_panes = (
    ...
    ('caffevis_aux',     (600,    0,  300,   300)),    # Zoom in to each filter
    ('caffevis_status',  (900,    0,   30,  1150)),
    ('caffevis_control', (  0,  300,   30,   870)),
    ('caffevis_layers',  ( 30,  300,  870,   870)),
    ('caffevis_jpgvis',  (  0, 1175,  900,   300)),
    ('caffevis_prob',    (300,    0,  200,   300)),    # Show list of probabilities

    # ANH: add panes for 
    # 1. face detector
    # 2. text detector
    # 3. circle detector
    ('caffevis_feature_detectors',    (900,    300,  30,   1150)),    # Show list of probabilities
)
...
global_scale = None
# global_scale = .9   # Original
global_scale = .8   # ANH modified
# Define global_font_size to scale all font sizes by this amount.
# global_font_size = .9
global_font_size = global_scale # ANH modified
...
# ANH: added positions of buttons for click interaction
buttons = {
    # Top buttons
    "conv1"   : (244, 310, 0, 25),
    "p1"      : (315, 345, 0, 25),
    ...
    # Bottom buttons
    "Face detector"    : (245, 395, 725, 745),
    "Text detector"    : (405, 550, 725, 745),
    "Circle detector"  : (560, 725, 725, 745),
    "Wrinkle detector" : (735, 920, 725, 745),
    "Edge detector"    : (930, 1077, 725, 745),
}

# Translation
voiceover = {
    "conv1" : "Layer one, convolution",
    "p1"    : "Layer one, pooling",
    "n1"    : "Layer one, normalization",
    "conv2" : "Layer two, convolution",
    "p2"    : "Layer two, pooling",
    "n2"    : "Layer two, normalization",
    "conv3" : "Layer three, convolution",
    "conv4" : "Layer four, convolution",
    "conv5" : "Layer five, convolution",
    "p5"    : "Layer five, pooling",
    "fc6"   : "Layer six, which is fully connected",
    "fc7"   : "Layer seven, which is fully connected",
    "fc8"   : "Layer seven, which is fully connected",
    "prob"  : "Final classification layer",

    "Synthetic images"  : "Synthetic images that maximally activate the highlighted neuron",
    "Real images"       : "Real images that maximally activate the highlighted neuron",
    "Deconv images"     : "For each of the 9 real images above that maximally activate the highlighted neuron, these images show which parts of those images caused that high activation",
    
    # Shortcuts
    "Face detector"     : "A face detector neuron. It will light up if a face is present.",
    "Text detector"     : "A text detector neuron. It will light up if some text is present.",
    "Circle detector"   : "A circle detector neuron. It will light up if a circle is present.",
    "Wrinkle detector"  : "A wrinkle detector neuron. It will light up if a wrinkle is present.",
    "Edge detector"     : "An edge detector neuron. It will light up if an edge is present.",
}